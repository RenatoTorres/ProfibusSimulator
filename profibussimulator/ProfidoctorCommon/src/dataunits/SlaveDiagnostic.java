...
public class SlaveDiagnostic extends GeneralDataUnit{
    ...
    public String toHTML( )
    {
        ...
        String sIdentHigh = Integer.toHexString( getIdentNumber_High() ).toUpperCase();
        String sIdentLow  = Integer.toHexString( getIdentNumber_Low() ).toUpperCase();
        
        if( sIdentHigh.length() == 1)
            sIdentHigh = "0" + sIdentHigh;

        if( sIdentLow.length() == 1)
            sIdentLow = "0" + sIdentLow;

        
        sret += "Ident Number: 0x" + sIdentHigh.substring(sIdentHigh.length()-2, sIdentHigh.length()) + sIdentLow.substring(sIdentLow.length()-2, sIdentLow.length()) + "<BR>";
        sret += "Master: " + String.valueOf( getMaster_Addr() ) + "<BR>";
