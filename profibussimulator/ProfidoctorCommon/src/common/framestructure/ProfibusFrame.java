...
public class ProfibusFrame
{
    ...
    */
    ...
    public Service getFrameServiceObject()
    {
        return frameService;
    }
...
//    }
    ...
    public float[][] BuildSignalDataSet(float BAUD_RATE, int SAMPLES_PER_BIT )
    {
        final int BITS_PER_BYTE = 11;
        final byte[] BData = getRawData();
        int samples = BData.length * BITS_PER_BYTE * SAMPLES_PER_BIT;
        byte bmask;
        int[] Bbits = new int[8];
        int   Bparity = 0;
        float fHighVoltage =  0;
        float fLowVoltage  = 5;
        float fsample_time = 0;
        final float SAMPLE_TIME_INTERVAL = (1/BAUD_RATE)/SAMPLES_PER_BIT;
        int S = 0;
        
        //DataSet with all samples [time_sample,value_sample]
        float[][] fDataSet = new float[samples][2];
        
        //Compute for all bytes in frame
        for(int i=0; i < BData.length; i++)
        {
            //Convert byte to bit array (Bbits)
            Bparity = 0;
            for(int j=0; j < 8; j++)
            {
                bmask = (byte) (0x01 << j);
                Bbits[j] = ( (BData[i] & bmask) != 0) ? 1 : 0;
                Bparity += (Bbits[j] != 0) ? 1 : 0;
            }
            
            //Compute StartBit
            for(int k=0; k < SAMPLES_PER_BIT; k++)
            {
                fDataSet[S][0]   = fsample_time;
                fDataSet[S++][1] = fLowVoltage;
                fsample_time += SAMPLE_TIME_INTERVAL;
            }
            
            //Compute samples to byte
            for(int j=0; j < 8; j++)
            {
                for(int k=0; k < SAMPLES_PER_BIT; k++)
                {
                    fDataSet[S][0]   = fsample_time;
                    fDataSet[S++][1] = (Bbits[j] != 0) ? fHighVoltage: fLowVoltage;
                    fsample_time += SAMPLE_TIME_INTERVAL;
                }
            }
            ...
            {
                fDataSet[S][0]   = fsample_time;
                fDataSet[S++][1] = ((Bparity % 2) == 0) ? fHighVoltage: fLowVoltage;
                fsample_time += SAMPLE_TIME_INTERVAL;
            }
...
//    }
    ...
    private void AssembleFrame(int address, int destAddr, byte function_service) {
        ...
        switch( frameService.getService() ) {
            ...
            case Service.PB_REQ_GLOBAL_CONTROL:
                frameSD.setSD(SD.SD2);
            break;
        ...
        switch( frameService.getService() ) {
            ...
            case Service.PB_REQ_GLOBAL_CONTROL:
                frameFC.setFC( FC.FC_REQ_SEND_DATA_WITH_NO_ACK_HIGH );
    ...
    {
        return frameFC.getFCB();
    }

    public boolean getFCV( )
    {
        return frameFC.getFCV();
    }
    ...
    public void setFC(byte fc_val)
    {
        frameFC.setFC(fc_val);
        Encode( );
    }    
}
